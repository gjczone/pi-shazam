#!/usr/bin/env bash
# agent-context-guard — PreToolUse hook: require structural context before Task (sub-agent) spawn
#
# Before spawning a Task (sub-agent) for non-trivial tasks, this hook checks
...
#
# CodeBuddy uses tool_name "Task" for sub-agent calls (not "Agent").
#
# stdin: { "hook_event_name": "PreToolUse", "tool_name": "Task", "tool_input": {...} }
...
if $is_review && [[ "$context_score" -lt 2 ]]; then
  echo "> Task (sub-agent) is a review/audit — consider providing file paths or symbol names from shazam_lookup for better results."
fi
...
if $is_coding && [[ "$word_count" -gt 50 ]] && [[ "$context_score" -lt 1 ]]; then
  echo "> Task (sub-agent) lacks file/symbol context — consider: shazam_lookup on target files, shazam_impact for multi-file changes"
fi
