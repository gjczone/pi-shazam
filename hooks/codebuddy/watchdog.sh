...
if [[ "$tool_name" == "Bash" ]]; then
  ...
  if [[ -n "$cmd" ]]; then
    ...
    # CodeBuddy has no PostToolUseFailure event — both success and failure
    # arrive as PostToolUse. Detect failure via error.message (when present)
    # or typical error markers in tool_output.
    error_msg=$(echo "$INPUT" | jq -r '.error.message // ""')
    tool_output=$(echo "$INPUT" | jq -r '.tool_output // ""')
    is_failure=false
    if [[ -n "$error_msg" ]]; then
      is_failure=true
    elif echo "$tool_output" | grep -qiE '(^|\b)(error|failed|fatal|traceback|exception|command not found|no such file|permission denied|denied|abort):'; then
      is_failure=true
