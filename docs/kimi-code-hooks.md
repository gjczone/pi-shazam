...
| `rename-state.ts`               | `impact-satisfied.sh`                         | Pi 内存标记；Kimi-Code 文件标记                                                               |
...
| -------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| v0.27.1        | v7（2026-07-10）     | Hooks 集中管理：所有 shell hooks 源码迁入项目 `hooks/codebuddy/`、`hooks/kimi/`；共享片段抽到 `hooks/lib/shazam-common.sh`（路径自检测）；`hooks/_shared.ts` 集中 pi 原生工具名常量；`scripts/deploy-hooks.sh` 单向部署到 ~ 目录；degrade safety（shazam_unavailable）双端统一；版本号从 hooks 内容中移除；新增 `docs/codebuddy-hooks.md` 开发文档。                                                                                                                                                                                                               |
| v0.27.0        | v6（2026-07-06）     | text -> JSON migration (#631 A)：7 个工具现在通过 buildXxxResult 暴露类型化的 XxxResult；lookup JSON 输出暴露 Edge.provenance (#643, "resolved" / "name_match" / "heuristic" / "unresolved")；LSP per-file mtime cache invalidation (#641)；.pi-shazam/config.json (#630)；shazam_lookup mode=state 完全移除；kimi-code shell hooks 无变更（工具名和 MCP 接口不变）                                                                                                                                                                                                |
| v0.27.0 (PR-G) | v6（2026-07-06）     | 4-tool enrichment (#631 B): shazam_impact per-affected-symbol provenanceCounts + mermaid call graph (CallChainEntry.mermaid), shazam_overview topByDensity (symbols/file ratio), shazam_lookup sort by provenance weight, shazam_changes structuralChanges (+/- line counts from git diff --numstat); V3 ProtoBuf cache (#628, ~30% smaller, magic header `SHA\3`, saveGraphCache writes V3 by default, loadGraphCache falls back to V2 JSON on unknown magic); kimi-code shell hooks 无变更（工具名和 MCP 接口不变；JSON 字段是 additive 扩展，不影响现有消费者） |
| v0.26.0        | v6（2026-07-03）     | 共享 MCP/Pi dispatch 层 (#618, 工具 _dispatchers.ts 是单一 source of truth)；MCP-Pi parity contract tests (#619, 19 个契约测试); check-mcp-parity.sh (#620, 10/10 PASS); kimi-code shell hooks 无变更（工具名和 MCP 接口不变）                                                                                                                                                                                                                                                                                                                                     |
| v0.25.0        | v6（2026-07-03）     | MCP robustness fixes (#596-#610): LSP cross-platform (PATHEXT, go/bin/.exe, pathToFileURL, symlink containment, per-language timeout), MCP error stack redaction, shazam_lookup existsSync file-path gate, onSignal setImmediate, Windows stdin error/close shutdown triggers; kimi-code shell hooks 无变更（工具名和 MCP 接口不变）                                                                                                                                                                                                                               |
| v0.24.4        | v5（2026-07-02）     | Cross-platform fixes: Windows CI, PATHEXT-aware executable detection, cache directory per platform, POSIX shell -> Node.js in package.json scripts, isExecutable ENOENT warning suppression; kimi-code shell hooks 无变更                                                                                                                                                                                                                                                                                                                                          |
...
1. [ ] 运行 `bash scripts/deploy-hooks.sh --apply` 部署最新 shell hooks 到 CodeBuddy 和 Kimi Code；运行 dry-run 确认无意外漂移
2. [ ] 检查 `mcp-reference.sh` 注入的工具列表是否与 `hooks/lib/shazam-common.sh` 一致
...
6. [ ] 检查 `pre-commit-verify.sh` 提醒逻辑是否正常工作（非阻断提醒）
7. [ ] 检查 `mcp-health.sh` 是否覆盖所有 7 个工具的失败回退，两端 degrade safety 是否都有（shazam_unavailable 标记）
8. [ ] 检查 `mcp-audit.sh` 审计日志路径和格式
9. [ ] 检查 `issue-guard.sh` + `pre-edit-impact-guard.sh` issue 门禁逻辑
10. [ ] 检查 `agent-context-guard.sh` 上下文检测规则
11. [ ] 运行 `bash -n` 语法检查所有脚本（deploy-hooks.sh 内置）
12. [ ] 更新本文档中的版本映射表
...
| 日期       | 内容                                                                                                                                                                                                              |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-06 | v0.27.0 发布：text -> JSON migration (#631 A) + Edge.provenance (#643) + LSP cache (#641) + .pi-shazam/config.json (#630) + mode=state 移除 (#630 cleanup)，kimi-code shell hooks 无变更                          |
| 2026-07-06 | v0.27.0 (PR-G) 发布：4-tool enrichment (#631 B) + V3 ProtoBuf cache (#628)，kimi-code shell hooks 无变更（JSON 字段是 additive 扩展，旧消费者不感知）                                                             |
| 2026-07-03 | v0.26.0 发布：共享 MCP/Pi dispatch 层 (#618) + parity tests (#619) + check-mcp-parity.sh (#620)，kimi-code shell hooks 无变更                                                                                     |
| 2026-07-03 | v0.25.0 发布：MCP robustness fixes (#596-#610)，kimi-code shell hooks 无变更                                                                                                                                      |
| 2026-07-02 | v0.24.4 发布：Cross-platform fixes（Windows CI, PATHEXT, cache dir），kimi-code shell hooks 无变更                                                                                                                |
