...
const server = new McpServer({ name: "pi-shazam", version: "0.27.1" });
...
| 2026-07-06 | 2.4     | v0.27.0: PR-G — 4-tool enrichment (#631 B): shazam_impact provenance breakdown + Mermaid call graph, shazam_overview topByDensity, shazam_lookup sort by provenance weight, shazam_changes structuralChanges. V3 ProtoBuf cache format (#628): ~30% smaller on-disk size, `core/graph.proto` schema, magic header `SHA\3`, `saveGraphCache` writes V3 by default, `loadGraphCache` reads V3 + falls back to V2. |
