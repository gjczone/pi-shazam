...
15. **Graph identity / diff key omits a field**: Any function that builds an edge/symbol identity string for `compareGraphSnapshots`, caching, or dedup MUST include every semantically-meaningful field (source, target, kind, weight, confidence, AND provenance). Dropping a field (e.g. `provenance` in `edgeIdentity`) silently hides real changes — `shazam_changes` reports zero diff when only the edge's trust level upgrades from `heuristic` to `resolved` (#695). When adding a field to `Edge`/`Symbol`, audit every identity/dedup/serialize path that consumes it.
...
19. **Status / policy observation routed to stderr instead of LLM context (issue #632 UX)**: A `_logWarn` that fires on a policy observation or status report -- NOT on a real failure -- is user-visible noise. Status information that the LLM agent needs must reach the agent via (a) the before-start hook's system-prompt injection (`hooks/before-start.ts:230`), (b) tool output text / JSON envelope, or (c) inline annotations like `(tree-sitter only, LSP unavailable)`. A parallel `console.warn` is duplicate signal the user does not need to see. Canonical surfaces: `core/overview.ts:91-96` renders `graph.truncated` as `[WARNING] File count exceeded MAX_FILES ...`; `core/overview.ts:280-286` renders the test-exclusion note. Reject any new `_logWarn` for "Excluded N test files", "MAX_FILES limit reached", "scan deadline exceeded", or analogous status / policy observations. The earlier removed `_logWarn`s live in commit `0457526` (test-exclusion) and `core/scanner.ts` MAX_FILES / scan-deadline paths; ensure they stay removed.
20. **Negative-probe ENOENT must be guarded caller-side (post-#551)**: The `_logWarn` framework (`core/output.ts:427`) does NOT suppress ENOENT -- the blanket early-return was deliberately removed in #551 because it hid real root causes. Callers that probe optional files / paths (e.g. an agent asking for a file that does not exist, MCP entry scanning for `package.json`, `validatePathInProject` checking containment) MUST add their own local guard before calling `_logWarn`:
    ```typescript
    const code = (err as NodeJS.ErrnoException).code;
    if (code === "ENOENT") return; // or `continue;` / `return false;` etc.
    _logWarn("tag", "context", err);
    ```
    Reference pattern: `core/scanner.ts:175`, `core/git-hooks.ts:307-313` (pre-existing), `tools/_factory.ts:74-77`, `core/path-utils.ts:136-138`, `mcp/entry.ts:120-122` (commit `67c66ca`). A new `_logWarn` callsite that may receive ENOENT from an optional probe without an explicit guard is a regression. Note: this rule covers the negative-probe case only; real filesystem failures (EACCES, ELOOP, ENOTDIR) MUST still log.
...
| `core/output.ts`  | `_logWarn`: framework does NOT suppress ENOENT (the blanket early-return was removed in #551 -- see `core/output.ts:403-432` JSDoc). Callers that probe optional files MUST add their own `if ((err as NodeJS.ErrnoException).code === "ENOENT") return;` guard before `_logWarn` (see P1 #20). Verify `_logWarn` is NOT used for status / policy observations -- those go to LLM context via the before-start hook (see P1 #19). `truncateOutput`: high-priority line detection by prefix -- verify important detail lines starting with `- ` are not dropped. |
...
- [ ] **Status / policy `_logWarn` vs LLM context (P1 #19)**: For any new `_logWarn` callsite, verify the underlying event is a real failure, NOT a status / policy observation. Status observations (test-exclusion counts, MAX_FILES / scan-deadline truncation, etc.) must reach the agent via `core/overview.ts`, the before-start hook's system-prompt injection, or an inline tool-output annotation. Reject duplicate-signal `_logWarn`s. Reference surfaces: `core/overview.ts:91-96` (truncated graph `[WARNING]`), `core/overview.ts:280-286` (test-exclusion note), `hooks/before-start.ts:166-180` (`buildParserWarningSection`).
- [ ] **Negative-probe ENOENT guard (P1 #20)**: For any new `_logWarn` callsite whose underlying call may receive ENOENT from an optional probe (file does not exist, package.json not found, etc.), verify the caller has an explicit `if ((err as NodeJS.ErrnoException).code === "ENOENT") return;` guard BEFORE the `_logWarn`. The `_logWarn` framework does NOT suppress ENOENT (post-#551). Reference pattern: `core/scanner.ts:175`, `core/git-hooks.ts:307-313`, `tools/_factory.ts:74-77`, `core/path-utils.ts:136-138`, `mcp/entry.ts:120-122`.
...
- `grep -rn "_logWarn" core/ tools/ lsp/ mcp/ --include="*.ts"` -- for every callsite, verify (a) the underlying event is a real failure (NOT a status / policy observation -- see P1 #19), and (b) if the call may receive ENOENT from an optional probe, the caller has an explicit `ENOENT` guard BEFORE `_logWarn` (see P1 #20). Reference valid sites: `core/scanner.ts:175`, `core/git-hooks.ts:307-313`, `tools/_factory.ts:74-77`, `core/path-utils.ts:136-138`, `mcp/entry.ts:120-122`.
