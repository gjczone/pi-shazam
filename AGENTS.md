...
- **Primary risk areas**: tree-sitter grammar version compat, LSP JSON-RPC framing, encoding fallback (UTF-8/GBK/GB2312), MCP/Pi tool-definition sync, Windows LSP discovery, V3 cache magic-byte routing, cache persistence status propagation, truncated-graph warning coverage, parser degraded-mode signaling.
...
- **Cache persistence status must propagate**: `saveGraphCache` returns a `CacheSaveResult` (not void) with `persisted: boolean` and optional `reason` / `sizeBytes`. The result is stored in `RepoGraph.cacheStatus` and surfaced in the overview so users know when the cache is not working (disk full, size limit exceeded, etc.). See #732, #733.
- **Truncation warnings must reach all tools**: When the graph is truncated (`MAX_FILES` or scan deadline), EVERY tool that consumes the graph must include the warning — not just `shazam_overview`. Users must know when results may be incomplete. See #731.
- **Parser degraded mode must be visible**: When individual tree-sitter query types fail to compile for a language, the degraded status is tracked in `_parserStatus.degradedQueries` and surfaced in the overview's "Parser Degraded Mode" section. Silent partial failures are not acceptable. See #734.
- **LSP file management has LRU safety net**: `LspClient.didOpen` enforces an LRU cap (`MAX_OPENED_FILES = 200`) to prevent unbounded LSP server-side AST accumulation. Enrichment functions (hover, definition, references) should explicitly close files after use, but the LRU cap is the safety net. See #729.
- **Shell hooks must source the shared library**: Every shell hook that uses `SHAZAM_LOG_DIR` or other shared variables MUST source `lib/shazam-common.sh` immediately after `set -euo pipefail`. Under `-u`, an unset variable crashes with exit code 1, which the hook framework interprets as "hook error" (fail-open). A missing source line turns a safety hook into a no-op. See #728.
- **Home-directory guard (#720)**: `mcp/entry.ts` and `index.ts` refuse to operate on a project root that equals or is nested under `$HOME` / `%USERPROFILE%`. Use `core/path-utils.ts:isHomeDirectory()` for the check (handles Windows case-insensitivity and the `HOME`/`USERPROFILE` fallback). Opt out with `PI_SHAZAM_ALLOW_HOME=1`. Pair this entry-level guard with the library-level `_walkDirectory` wall-clock deadline (`PI_SHAZAM_SCAN_DEADLINE_MS`, default `10000` ms) so an adversarial directory tree cannot hang the scanner past MCP's default 30 s timeout.
